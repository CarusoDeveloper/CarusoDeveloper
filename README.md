# Olá, sou o Caruso! 👋

### 🚀 Software Engineer | Backend & Systems Focus
Sou apaixonado por construir sistemas robustos e escaláveis. Meu foco principal é o desenvolvimento de arquiteturas eficientes, lógica complexa e otimização de performance no lado do servidor.

---

### 🛠 Tech Stack (Backend Ecosystem)

<div align="left">
  <img src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white" />
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white" />
  
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
</div>

---

### ⚙️ No que eu foco:
- 🛡️ **Arquitetura:** Clean Code e estruturação de APIs.
- ⚡ **Performance:** Otimização de algoritmos e consumo de memória.
- 🗄️ **Dados:** Modelagem de bancos de dados e integração.

---

### 📊 GitHub Insights

<p align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=CarusoDeveloper&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true" />
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=CarusoDeveloper&layout=compact&langs_count=7&theme=tokyonight" />
</p>

<p align="center">
  <img width="100%" src="https://github-readme-streak-stats.herokuapp.com/?user=CarusoDeveloper&theme=tokyonight" />
</p>

